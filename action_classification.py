...
parser.add_argument('--test_modality', default='joint', type=str,
                    help='traiining protocol of ntu')
parser.add_argument('--save_path', default='dummy', type=str,
                    help='traiining protocol of ntu')
...
def main():
    ...
    wandb.init(project="halp",config=args,tags=['lineval'],settings=wandb.Settings(start_method="fork"))
    if wandb.run.sweep_id is None:
        folder_name = 'runs'
    else:
        folder_name = wandb.run.sweep_id
    args.save_path = f'checkpoints/{folder_name}/{wandb.run.name}'
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = moco.builder_mm_cmd.MoCo(args.finetune_skeleton_representation, opts.bi_gru_model_args, pretrain=False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch+1) % 5 == 0:
            acc1,logits = validate(val_loader, model, criterion, args)
        else:
            acc1 = 0
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output
        output = model(images,view=args.test_modality)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, Path(filename).parent / 'model_best.pth.tar')
